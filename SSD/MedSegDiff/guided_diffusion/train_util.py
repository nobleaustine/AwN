...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        l2 = time()
        print(f"Time taken for loading data is {l2-l1}")
        while (
            ...
            try:
                   # getting batch from dataset 
                    ...
                    d2 = time()
                    print(f"Time taken for batch {i} is {d2-d1}")
            ...
            t2 = time()
            print(f"Time taken for step {i} is {t2-t1}")
...
# exploring: 4
    def forward_backward(self, batch, cond):
        ...
        # cleaning gradient before recomputing
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # checking if last batch or not
            last_batch = (i + self.microbatch) >= batch.shape[0]
            # sampling the time for each image in batch
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())
            
            # wrapped using functools to avoid passing arguments calculating loss
