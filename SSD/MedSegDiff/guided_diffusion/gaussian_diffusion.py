...
def standardize(img):
    """
    img(N, C, H, W): input image
    return:img(N, C, H, W): standardized image
    img= (img - mean)/std
    """
    mean = th.mean(img)
    std = th.std(img)
...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    """
    shedule_name(str): name of the beta schedule
    num_diffusion_timesteps(int): number of diffusion steps
    return:betas(num_diffusion_timesteps,): beta schedule
    [beta_start, beta_start + (beta_end - beta_start) / num_diffusion_timesteps, ...,beta_end]
...
def betas_for_alpha_bar(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    """
    num_diffusion_timesteps(int): number of diffusion steps
    alpha_bar(lambda): alpha_bar function
...
class ModelVarType(enum.Enum):
    """
    LEARNED: the model predicts the log-variance.
    LEARNED_RANGE option has been added to allow the model to predict.
...
class LossType(enum.Enum):
    """
    KL: the variational lower-bound
    ...
    BCE_DICE: use BCE loss and DICE loss
    """
    ...
    # checking if it is varitional bound 
    def is_vb(self):
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        # speedup and modification parameters
        self.rescale_timesteps = rescale_timesteps
        self.dpm_solver = dpm_solver

        # checking and setting up betas
        ...
        # all required values
        alphas = 1.0 - betas # 1- b_t
        self.alphas_cumprod = np.cumprod(alphas, axis=0) # A_t
        # a = 1 -b if b_1 = 0.0001, then a_0= 1- b_0(smaller than b1) is approx 1
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1]) # A_{t-1} 
        # A_1000 = P(t,1,1000)a_t approx 0 , then A_1001 is approx 0
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0) # A_{t+1}
        
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,) ==self.alphas_cumprod_next.shape,"cross check new alphas and time step failed"
        ...
        # beginning of the diffusion chain so seocond value replaced as first value
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        # using formula of the foreward process
        return (
                _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        # weighted some of x_0 and x_t is x_(t-1) mean
        posterior_mean = (
    ...
    def p_mean_variance(
        ...
        """
        # model is dummy need to know more
        ...
        cal = 0
        assert t.shape == (B,)
        # model output with mean and variance 
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # splitting into episilon and variance
            model_output, model_var_values = th.split(model_output, C, dim=1)
            if self.model_var_type == ModelVarType.LEARNED:
                model_log_variance = model_var_values
                model_variance = th.exp(model_log_variance)
            else:
                # equation 15 componets SIGMA(xt,t) = exp(v(log(beta_t)+(1-v)logtilda_beta_t))
                # the log terms
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                # predict x_t start given noise image
                pred_xstart = process_xstart(
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ...
        x_t(B,C,H,W): input batch of noised images
        t: time steps of each image
        xprev(B,C,H,W): noised image of previous time step

        x_0(B,C,H,W): start of each noise images(unnoised images)
        
        x_0 = 1/(b*(1-A_{t-1}/(1-A_t))*x_{t-1} - [(1-A_{t-1})(sqrt(a_t)/(1-A_t)]/[A_t -1)]/[(b*(1-A_{t-1}/(1-A_t))*x_{t-1}]x_t
        """
    ...
    def _scale_timesteps(self, t):
        """"scale timesteps from 1000 to num_timesteps"""
    ...
    def p_sample_loop_progressive(
        ...
        if device is None:
            device = next(model.parameters()).device
        assert isinstance(shape, (tuple, list)),"shape should be tuple or list"
    ...
    def _vb_terms_bpd(
        ...
        """
        # foreward process posterior
        true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(
            x_start=x_start, x_t=x_t, t=t
        )
        # similarly for the p distribution
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        # kl divergence between that and learned reverse process
        kl = normal_kl(
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        # doing q sample generating q(xt|x0)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                # not episilon only variance
                frozen_out = th.cat([model_output.detach(), model_var_values], dim=1)
                # for variance and mean different loss function
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    # extracting by time step alpha
    res = th.from_numpy(arr).to(device=timesteps.device)[timesteps].float()
    while len(res.shape) < len(broadcast_shape):
        res = res[..., None]
    # setting up same shape as image for pointwise multiplication i guess
    # scalar tensor multiplication not possible
