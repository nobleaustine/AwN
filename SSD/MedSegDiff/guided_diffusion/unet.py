...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        assert x.shape[1] == self.channels,"input channels != model channels"
        if self.dims == 3:
            x = F.interpolate(
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        if use_conv:
            ...
            assert self.channels == self.out_channels,"input channels != output channels"
...
class ResBlock(TimestepBlock):
    ...
    :param channels: the number of input channels. **RGB
    :param emb_channels: the number of timestep embedding channels. **time embedding vector dimention
    :param dropout: the rate of dropout.
    :param out_channels: if specified, the number of out channels. **output channels
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            # preprocessing like passing through some layers
            h = self.in_layers(x)
        emb_out = self.emb_layers(emb).type(h.dtype)
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None]
        # adding temporal info to image channels    
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]
            scale, shift = th.chunk(emb_out, 2, dim=1)
            h = out_norm(h) * (1 + scale) + shift
            h = out_rest(h)
            # adding temporal info in a new way
        else:
            h = h + emb_out
            h = self.out_layers(h)
...
class UNetModel_v1preview(nn.Module):
    ...
    :param in_channels: channels in the input Tensor: **eg: RGB 
    :param model_channels: base channel count for the model. 
    :param out_channels: channels in the output Tensor. **eg 128-> 256-> 512 or reverse
    ...
    :param channel_mult: channel multiplier for each level of the UNet. **how much channel increase
    ...
    def __init__(
        ...
        # mlp layer with a SiLU for creating a time embedding ns
    ...
    def forward(self, x, timesteps, y):# y = None
        """
...
class UNetModel_newpreview(nn.Module):
    ...
    def forward(self, x, timesteps, y = None): # y= None
        """
        Apply the model to an input batch.
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   step_size: float = 0.5, patch_size: Tuple[int, ...] = (), regions_class_order: Tuple[int, ...] = (),
                   use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = {}, all_in_gpu: bool = False,
