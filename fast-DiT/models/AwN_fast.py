...
class MMBlock(nn.Module):
    ...
    #     gate_msa_unsqueezed = gate_msa.unsqueeze(1)
    #     gate_mlp_unsqueezed = gate_mlp.unsqueeze(1)
        
    #     dx, dy = self.attn(modulate(self.norm1(x), shift_msa, scale_msa), modulate(self.norm1(y), shift_msa, scale_msa))
        
    #     x.addcmul_(gate_msa_unsqueezed, dx)
    #     y.addcmul_(gate_msa_unsqueezed, dy)
        
    #     modulated_x_mlp = modulate(self.norm2(x), shift_mlp, scale_mlp)
    #     modulated_y_mlp = modulate(self.norm2(y), shift_mlp, scale_mlp)
        
    #     x.addcmul_(gate_mlp_unsqueezed, self.mlp(modulated_x_mlp))
    #     y.addcmul_(gate_mlp_unsqueezed, self.mlp(modulated_y_mlp))
...
class awn(nn.Module):
    ...
    def initialize_weights(self):
        ...
        nn.init.constant_(self.final_layer_1.adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_layer_1.adaLN_modulation[-1].bias, 0)
        nn.init.constant_(self.final_layer_1.linear.weight, 0)
        nn.init.constant_(self.final_layer_1.linear.bias, 0)

        nn.init.constant_(self.final_layer_2.adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_layer_2.adaLN_modulation[-1].bias, 0)
        nn.init.constant_(self.final_layer_2.linear.weight, 0)
        nn.init.constant_(self.final_layer_2.linear.bias, 0)
    ...
    def forward(self, x, t, y):
        ...
        """
        x = self.x_embedder(x).add_(self.pos_embed)  # (N, T, D), where T = H * W / patch_size ** 2
        t = self.t_embedder(t)                   # (N, D)
        y = self.y_embedder(y).add_(self.pos_embed) # (N, T, D)
...
def awn_S_8(**kwargs):
    return awn(depth=6, hidden_size=384, patch_size=8, num_heads=6, **kwargs)

def awn_S_32(**kwargs):
    return awn(depth=6, hidden_size=384, patch_size=16, num_heads=6, **kwargs)
