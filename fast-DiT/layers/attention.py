...
class SegAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor,y: torch.Tensor) -> torch.Tensor:
        ...
        i_q = i_q * i_scale
        n_attn = torch.matmul(i_q, l_k.transpose(-2, -1))
        ...
        l_q = l_q * l_scale
        f_attn = torch.matmul(l_q, i_k.transpose(-2, -1))
        ...
        x = torch.matmul(self.scalar_param * n_attn, l_v) # + torch.matmul((1 - self.scalar_param) * an_attn, i_v)
        x = x.transpose(1, 2).reshape(N, T, D)

        y = torch.matmul(self.scalar_param * f_attn, i_v) # + torch.matmul((1 - self.scalar_param) * af_attn, l_v)
